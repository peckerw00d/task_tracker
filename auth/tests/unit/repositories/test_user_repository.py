...
@pytest.mark.asyncio
async def test_repository_get_by_email(user_repo, test_user, mock_async_session):
    mock_async_session.execute.return_value.scalars.return_value.first.return_value = (
        test_user
    )

    result = await user_repo.get_by_email("example@example.com")

    actual_query = str(mock_async_session.execute.await_args[0][0])
    expected_query = str(select(User).where(User.email == "example@example.com"))
...
@pytest.mark.asyncio
async def test_repository_get_by_email_not_found(user_repo, mock_async_session):
    mock_async_session.execute.return_value.scalars.return_value.first.return_value = (
        None
    )
